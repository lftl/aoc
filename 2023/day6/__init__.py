...
def solve(inp=None):
    ...
    if inp is None:
        inp = common.load_input(6)
    inp = inp.splitlines()

    # Parse the input to get the race times and record distances
    race_times = []
    record_distances = []
    race_times = inp[0].split(':')[1].split()
    record_distances = inp[1].split(':')[1].split()

    total = 1
    for i in range(len(race_times)):
        max_hold_time, min_hold_time = calculate_times(race_times[i], record_distances[i])
...
def calculate_times(time, record):
    ...
    disc = (time**2 - 4*(-1)*(-record))**0.5
    max_hold_time = floor((-time - disc)/-2)
    min_hold_time = ceil((-time + disc)/-2)

    if max_hold_time * (time - max_hold_time) == record:
        max_hold_time -= 1
        min_hold_time += 1
...
def solve2(inp=None):
    ...
    max_hold_time, min_hold_time = calculate_times(time, record)
